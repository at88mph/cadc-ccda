  ...
  function UnitSelection(options) {
    ...
    function init(grid) {
      ...
      _handler
        ...
        .subscribe(
          _grid.onBeforeHeaderRowCellDestroy,
    ...
    function handleHeaderRowCellRendered(e, args) {
      ...
      if (column.header && column.header.units) {
        ...
        while (i--) {
          ...
          var nextOption = $('<option></option>')
            .appendTo(unitPullDown)
            .text(unitDefinition.label)
            .val(unitDefinition.value)
    ...
    function handleBeforeHeaderRowCellDestroy(e, args) {
      ...
      if (column.header && column.header.units) {
        ...
        $(args.node)
          .find('.' + options.unitPulldownCssClass)
          .remove()
      }
    }
