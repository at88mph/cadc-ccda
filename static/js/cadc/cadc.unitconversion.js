 ...
 */
 (function() {
    ...
    function formatNumericFixation(value, fixation) {
        var numberFormat = new cadc.NumberFormat(value, fixation)
        return numberFormat.formatFixation()
    }

    function formatNumericPrecision(value, precision) {
        var numberFormat = new cadc.NumberFormat(value, precision)
    ...
    function FrequencyConverter(_value, _toUnit) {
        const _selfFrequencyConverter = this
        ...
        function getFrequencyFactor(unit) {
            const unitEntry = ca.nrc.cadc.search.unitconversion.energy.freqUnits[unit]
            return unitEntry ? unitEntry.factor : null
        }
        ...
        function normalizeFrequency() {
            return Number(getValue() * (1.0 / getFrequencyFactor(getToUnit())))
        }
        ...
        function rebase(_fromUnit) {
            const factor = getFrequencyFactor(_fromUnit.toLowerCase())

            if (factor) {
                const frequency = Number(getValue())
    ...
    function MJDConverter(_date) {
         ...
         */
        function convert() {
            return (_date / 86400000 - _date.getTimezoneOffset() / 1440 + 2440587.5 - 2400000.5)
        }
    ...
    function DateConverter(_value, _toUnit) {
         ...
         */
        function formatMJD() {
            const MJD = Number(getValue())

            // Julian day
            const jd = Math.floor(MJD) + 2400000.5

            // Integer Julian day
            let jdi = Math.floor(jd)

            // Fractional part of day
            const jdf = Number(jd - jdi + 0.5)
            ...
            let fraction = Number(MJD - Math.floor(MJD))
            const hours = Math.floor(Number(fraction * 24.0))
            fraction = fraction * 24.0 - hours

            const minutes = Math.floor(Number(fraction * 60.0))
            fraction = fraction * 60.0 - minutes

            const seconds = Math.floor(Number(fraction * 60.0))
            fraction = fraction * 60.0 - seconds

            const milliseconds = Number(fraction * 1000.0).toFixed(0)
            let l = jdi + 68569
            ...
            const dateFormat = includeTime() ? ca.nrc.cadc.search.formats.date.ISO : ca.nrc.cadc.search.formats.date.W3C
            const dateFormatter = new ca.nrc.cadc.search.DateFormat(date, dateFormat)
        ...
        function convertDate() {
            return isIVOAToUnit() ? formatMJD() : formatNumericFixation(getValue(), 5)
        }
    ...
    function AngleConverter(_value, _toUnit) {
        const _selfAngleConverter = this
        ...
        function getAngleFactor(_angleUnit) {
            const unitEntry = ca.nrc.cadc.search.unitconversion.angle[_angleUnit]
            return unitEntry ? unitEntry.factor : null
        }
        ...
        function convertAngle() {
            const angle = Number(getValue())
            const toFactor = getAngleFactor(getToUnit())

            return toFactor ? Number(toFactor * angle).valueOf() : null
        }
         ...
         */
        function rebase(_fromUnit) {
            const factor = getAngleFactor(_fromUnit)
            const area = Number(getValue())
         ...
         */
        function convert() {
            const res = isToAngleUnit() ? convertAngle() : null
    ...
    function AreaConverter(_value, _toUnit) {
        ...
        function getAreaFactor(_areaUnit) {
            const unitEntry = ca.nrc.cadc.search.unitconversion.area[_areaUnit]
            return unitEntry ? unitEntry.factor : null
        }
        ...
        function convertArea() {
            const area = Number(getValue())
            const factor = getAreaFactor(getToUnit())
            return factor ? Number(factor * area).valueOf() : null
        }
         ...
         */
        function convert() {
            const res = isToAreaUnit() ? convertArea() : null
            return formatNumericFixation(res, 4)
        }
