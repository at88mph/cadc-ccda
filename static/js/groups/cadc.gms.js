;(function($, undefined) {
  ...
  function GroupManager(_resourceEndpoint) {
    ...
    this.gridTranslationsContainer = document.getElementById(`list_content_headers`)
    ...
    function translateField(_entry) {
      const childTranslations = _selfGroupManager.gridTranslationsContainer.children
      const len = childTranslations.length
      for (let i = 0; i < len; i++) {
        const nextTranslation = childTranslations[i]
        if (nextTranslation.classList.contains(_entry)) {
          return nextTranslation.innerHTML
        }
      }
